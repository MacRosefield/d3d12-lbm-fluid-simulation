...
VertexShaderOutput VS_main(VertexInput input)
{
    ...
    float4 worldPosition = mul(modelViewMatrix, float4(input.position, 1.0f)); // Model to Camera space
    output.position = mul(projectionMatrix, worldPosition); // Camera space to Clip space
