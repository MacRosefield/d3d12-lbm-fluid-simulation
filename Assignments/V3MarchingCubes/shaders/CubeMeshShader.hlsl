...
)
{
    ...
    uint caseIndex = (threadId.x + threadId.y) % 256;
    uint edgeBits = edgeTable[caseIndex];
...
float4 PSMain(VertexOut input) : SV_Target
{
    ...
    int edgeIndex = triTable[50 * 16 + 0];
    float norm = (edgeIndex == -1) ? 0.0f : edgeIndex / 11.0f; // max edgeIndex = 11
    
    
    return float4(norm, input.color.y, edgeTable[50], 1.0);
