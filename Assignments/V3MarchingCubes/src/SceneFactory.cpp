                                               ...
                                               const Microsoft::WRL::ComPtr<ID3D12CommandQueue>& commandQueue)
{
  ...
  createBoundingBoxes(device, commandQueue, outputScene);
  ...
  outputScene.m_quad = FullScreenQuad(device, commandQueue);
...
void SceneGraphFactory::createBoundingBoxes(const Microsoft::WRL::ComPtr<ID3D12Device>&       device,
                                       const Microsoft::WRL::ComPtr<ID3D12CommandQueue>& commandQueue,
                                       Scene&                                            outputScene)
{
  ...
  for (const TriangleMeshD3D12& mesh : outputScene.m_meshes)
  {
    ...
    outputScene.m_BoundingBoxes.emplace_back(mesh, device, commandQueue);
  }
}
