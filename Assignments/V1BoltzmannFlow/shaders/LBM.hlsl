...
// === Wände oder Hindernisse: keine Kollision ===
    ...
    */
    ...
    if (cellType == 1 )
    {
        float2 inletU = float2(0.15f, 0.0f);
        float inletRho = 1.0f;
        float u2 = dot(inletU, inletU);
        ...
        for (int i = 0; i < 9; i++)
        {
            float cu = dot(c[i], inletU);
            cell.f[i] = w[i] * inletRho * (1.0 + 3.0 * cu + 4.5 * cu * cu - 1.5 * u2);
        }

        b_gridWrite[index] = cell;

        g_OutputTexture[threadID.xy] = float4(0.0, 0.0, 1.0, 1.0); // weiß = Inflow
        return;
    }
